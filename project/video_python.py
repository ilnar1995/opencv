...
def func(frame, dim, frame_time, fps, zoom):
    ...
    def move_zoom_frame(frame, effectName, dim, frame_time, fps, i):
        k = i / frame_time * 1000 / fps
    ...
    if effectName == "zoomIn" or effectName == "zoomOut":
        ...
        for i in range(quantity_frame):
            resized = move_zoom_frame(frame, effectName, dim, frame_time, fps, i)  # обрезка кадра для i-го изображения
            out.write(resized)
    else:
        for i in range(quantity_frame):
...
effectName = "moveLeft"       # zoomIn        zoomOut     moveTop  moveLeft  moveRight  moveBottom
if not zoom:
    zoom = 1
if not frame_time:
    frame_time = 1000
...
func(frame, dim, frame_time, fps, zoom)
